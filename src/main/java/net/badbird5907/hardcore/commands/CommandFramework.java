...
import net.badbird5907.hardcore.object.Cooldown;
...
public class CommandFramework implements CommandExecutor {
	...
	public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		for (int i = args.length; i >= 0; i--) {
			...
			String cmdLabel = buffer.toString().replaceFirst(Hardcore.getInstance().getDescription().getName().toLowerCase() + ":","").replaceFirst("/" + Hardcore.getInstance().getDescription().getName().toLowerCase() + ":","");
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (!command.permission().equals(""))
					if (!sender.hasPermission(command.permission())) {
						sender.sendMessage(CC.RED + "You don't have permission!");
						return true;
					}
				if (command.playerOnly()) {
					if(!(sender instanceof Player)){
						sender.sendMessage(CC.RED + "This is player only!");
						return true;
					}
				}
				...
				if(command.cooldown() >= 1){
					if(sender instanceof Player){
						...
						if(Cooldown.isOnCooldown(command.name() + "|cmd_cooldown", p.getUniqueId())){
							p.sendMessage(CC.RED + "Please wait a bit!");
							return true;
						}
		...
		sender.sendMessage(CC.RED + "Unhandled command!");
		return true;
	}
	/**
	...
	public void registerCommand(Command command, String label, Method m, Object obj) {
		...
		if(!(command.permission()==null)&&!(command.permission() =="")&&!(command.permission() == "")){
